...
OBSTACLE_POS = [[38.5, 13.5], [9.5, 17.3], [26.2, 39.5]]
OBSTACLE_RADIUS = [2.5, 2.5, 2.5]
...
#         self.action_space = spaces.Box(self.action_low, self.action_high, dtype = np.float16)
    ...
    def step(self, action):
        ...
        velocity_surge_a, velocity_yaw_a, target_distance_a, target_radian_a, ahead_radian_a,\
        velocity_surge_b, velocity_yaw_b, target_distance_b, target_radian_b, ahead_radian_b= self.state # 获取agent位置，速度，目标位置
        ...
        self.done_a = bool(target_distance_a <= self.min_obs_dis or self.t > self.total_step)   
        ...
        if target_distance_b <= self.min_obs_dis:
            ...
            #     dense_reward += 1
            # self.reward_b = self.reward_b + DECRIE * dense_reward
             
        

        self.state = (velocity_surge_a, velocity_yaw_a, target_distance_a, target_radian_a, ahead_radian_a,
        velocity_surge_b, velocity_yaw_b, target_distance_b, target_radian_b, ahead_radian_b)
    ...
    def reset(self):
        ...
        while self._is_valied(cur_x, cur_y, test_list) is not True:
            cur_x, cur_y = self._random_pos()
        self.usv_x_a, self.usv_y_a = cur_x, cur_y
        ...
        while self._is_valied(cur_x, cur_y, test_list) is not True:
            cur_x, cur_y = self._random_pos()
        self.usv_x_b, self.usv_y_b = cur_x, cur_y
        test_list.append([cur_x, cur_y])
...
#         self.control_out = 0
        ...
        self.state = np.array([0.0, 0.0, target_distance_a, target_radian_a, ahead_radian_a, 
        0.0, 0.0, target_distance_b, target_radian_b, ahead_radian_b])
