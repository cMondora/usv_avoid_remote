...
#         self.control_out = 0
        # 作为观察空间每一个特征值的下限
        self.low = np.array([0,                      # surge 速度 0.0 ~ 2.0
        ...
        self.high = np.array([self.max_surge,
                            self.max_yaw,          
                            100.0 * math.sqrt(2),
                            math.pi,    
                            2 * math.pi,
...
#         self.action_space = spaces.Box(self.action_low, self.action_high, dtype = np.float16)
    ...
    def step(self, action):
        ...
        velocity_surge_m, velocity_yaw_m, target_distance_m, target_radian_m, ahead_radian_m, \
        velocity_surge_n, velocity_yaw_n, target_distance_n, target_radian_n, ahead_radian_n = self.state # 获取agent位置，速度，目标位置
        ...
        done = bool(target_distance_m <= self.min_obs_dis or target_distance_n <= self.min_obs_dis or self.t > self.total_step)   

        self.reward = 0.0

        if target_distance_m <= self.min_obs_dis and target_distance_n <= self.min_obs_dis:
            self.reward = 1 + 500 / self.t
        elif target_distance_m <= self.min_obs_dis:
            self.reward = 1 + 500 / self.t - target_distance_n / 15
        elif target_distance_n <= self.min_obs_dis:
            self.reward = 1 + 500 / self.t - target_distance_m / 15
        elif self.t > self.total_step - 1:
            self.reward = 1 - target_distance_m / 15 - target_distance_n / 15

        
        self.state = (velocity_surge_m, velocity_yaw_m, target_distance_m, target_radian_m, ahead_radian_m, \
        velocity_surge_n, velocity_yaw_n, target_distance_n, target_radian_n, ahead_radian_n)
...
#         self.control_out = 0
        ...
        self.state = np.array([velocity_surge_m, velocity_yaw_m, target_distance_m, target_radian_m, ahead_radian_m, \
        velocity_surge_n, velocity_yaw_n, target_distance_n, target_radian_n, ahead_radian_n])
    ...
    def render(self, mode='human', close=False):
        ...
        if self.viewer is None:
            ...
            self.agent_m = rendering.make_circle(2.5 * self.scale, 30, True)
            ...
            self.agent_n = rendering.make_circle(2.5 * self.scale, 30, True)
