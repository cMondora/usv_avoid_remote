...
#         self.action_space = spaces.Box(self.action_low, self.action_high, dtype = np.float16)
    ...
    def step(self, action):
        ...
        if self.done_a == False:
            ...
            dense_reward = 0.0
            if is_obstacle == 0 and target_distance < old_target_distance:
                dense_reward += 1
            sum_ob = sum(obstacle)
            if is_obstacle == 1 and old_is_obstacle == 1 and sum_ob > old_sum_ob:
                dense_reward += 1
            if is_obstacle == 0 and old_is_obstacle == 1:
                dense_reward += 1
