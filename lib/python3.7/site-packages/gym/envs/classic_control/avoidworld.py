...
class AvoidWorldEnv(gym.Env):
    ...
    def step(self, action):
        ...
        acc_surge_num, acc_yaw_num = action    # action for rendering
        ...
        for pos in OBSTACLE_POS:
            ...
            if obs_dis <= self.min_obs_dis:
                is_crash = True
        if is_crash:
            self.reward =  self.reward - 100.0 # give an reward
        if velocity_surge == self.max_surge:
            self.reward = self.reward + 1.0     # encourage velocity
        if acc_surge_num != 1:
            self.reward = self.reward - 1.0     # discourage acc
        if acc_yaw_num != 2:
            self.reward = self.reward - 1.0
        if target_distance <= self.min_obs_dis:
            self.reward = self.reward + 100.0
    ...
    def render(self, mode='human', close=False):
        ...
        if self.viewer is None:
            ...
            for obstacles_pos in OBSTACLE_POS:
                obstacle = rendering.make_circle(2.5 * self.scale, 30, True)
