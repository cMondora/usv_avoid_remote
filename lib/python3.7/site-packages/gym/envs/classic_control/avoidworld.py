...
#         self.action_space = spaces.Box(self.action_low, self.action_high, dtype = np.float16)
    ...
    def step(self, action):
        ...
        if self.done_a == False:
            ...
            if is_crash_a:
                self.reward_a += - 1.0 - target_distance_a / 15
            if target_distance_a <= self.min_obs_dis:
                self.reward_a += 1 + 500 / self.t
            if self.t > self.total_step:
                self.reward_a += 1 - target_distance_a / 15 
            dense_reward = 0.0
            ...
            if is_obstacle_a == 0 and old_is_obstacle_a == 1:
                dense_reward += 1
            self.reward_a = self.reward_a + DECRIE * dense_reward
        else:
        ...
        if self.done_b == False:
            ...
            if is_crash_b:
                self.reward_b += - 1.0 - target_distance_b / 15
            if target_distance_b <= self.min_obs_dis:
                self.reward_b += 1 + 500 / self.t
            if self.t > self.total_step:
                self.reward_b += 1 - target_distance_b / 15
        ...
        self.reward = self.reward_a + self.reward_b    
